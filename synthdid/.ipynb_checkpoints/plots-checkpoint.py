...
class Plots:
    ...
    def plot_outcomes(self, times = None, time_title_cb = int, labels=None, wtplot=True, axlimit_zero=False):
        ...
        for i, time in enumerate(times):
            ...
            fig, ax = plt.subplots()
            ax.plot("time", "control", label='Controllo sintetico', data=trajectory, linestyle="--", alpha=0.8, color='#6a79f7')
            ax.plot("time", "treatment", label=self.data[self.data[self.treatment]==1][self.unit].values[0], data=trajectory, alpha=0.8, color='#b9484e')                    
